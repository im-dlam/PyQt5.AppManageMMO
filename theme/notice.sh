...
THEME_DIR="$(pwd)"
APP_DIR="../models/app"
UI_FILE="${THEME_DIR}/notice.ui"
PY_FILE="${THEME_DIR}/notice.py"
...
if [ $? -eq 0 ]; then
  ...
  echo "Di chuyển file notice.py từ theme sang app..."
  mv "$PY_FILE" "$APP_DIR/notice.py"
  echo "File notice.py đã được chuyển đến thư mục $APP_DIR."
else
  echo "Lỗi: Không thể tạo file notice.py từ notice.ui."
fi
